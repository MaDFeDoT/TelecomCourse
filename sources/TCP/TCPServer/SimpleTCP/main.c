...
void Watch_Rate(void* newsockfd)
{
    ...
    for(i=0; i<Koll_Rate; i++)
    {
        ...
        {
            snprintf(buf, 500, "%s %d %c %d %d$\n", All_Rates[i].Name, All_Rates[i].id_pl, All_Rates[i].type[0], All_Rates[i].number, All_Rates[i].summa);
            //snprintf(buf, 60, "%d", All_Rates[i].number);
            //write(newsockfd, buf, strlen(buf));
            //write(newsockfd, " ",1);
        }
        else
            snprintf(buf, 500, "%s %d %c %d$\n", All_Rates[i].Name, All_Rates[i].id_pl, All_Rates[i].type[0], All_Rates[i].summa);
